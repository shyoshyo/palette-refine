...
int main(int argc, char** argv) {
	...
	Mat img = load_image(argv[2]);
	if (img.cols == 0) {
		std::cout << "Load from " << argv[2] << " failed.\n";
		return 0;
	}
	...
	for (int i = 0; i < sample_height; ++i)
		for (int j = 0; j < sample_width; ++j)
		{
			...
			p[np][0] = pointer[3 * y + 2] / 255.0f;
			p[np][1] = pointer[3 * y + 1] / 255.0f;
			p[np][2] = pointer[3 * y] / 255.0f;
			++np;
		}


	printf("nv = %d, nf = %d, np = %d\n", nv, nf, np);
